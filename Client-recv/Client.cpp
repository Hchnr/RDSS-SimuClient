...
int Client::userRecvWait(unsigned int id, unsigned short frame)
{
    unsigned char buf[BUFFER_LENGTH] = {
    0x00, 0x00, 0x00, 0x00, /* 4 dest user */
    0x00, 0x00, 0x00, 0x01, /* 8 src user */
    0x00, 0xff,             /* 10 frame id */
    0x80, 0x00,             /* 12 OPTION */
	0x00, 0x00,             /* 14 frame id */
    0x00,                   /* 15 OPTION kind */
    0x00,                   /* 16 OPTION hint */
    };

    /* SET dest id id*/
    unsigned int *pDest = (unsigned int *) (buf + 4);
    *pDest = htonl(id);

    /* SET src id id*/
    unsigned int *pSrc = (unsigned int *) (buf + 0);
    *pSrc = htonl(m_userID);
    ...
    *pOpt = 0x80;
    ...
    /* SET frame */
    unsigned short *pF = (unsigned short *) (buf + POS::option+2);
    *pF = htons(frame);

    /* SET option kind */
    unsigned char *pKind = buf + POS::option + 4;
    *pKind = 0x60;

    int packet_len = 4 + POS::data;
    printf("[send packet to: %s %d]\n", m_channelIP.data(), m_channelPort);
    for(int i = 0; i < packet_len; i++)
         printf("0x%02X ", buf[i]);
    printf("\n");
...
int Client::userRecv()
{
    ...
    while(1) {
        ...
        */
        ...
        if(*(buf + option) == 0x60) {
            userRecvWait(id, ntohs(*(unsigned short*) (buf+frame_id)));
            continue;
        }
...
{
    unsigned char buf[BUFFER_LENGTH] = {
    0x00, 0x00, 0x00, 0x02, /* 4 dest user */ 
