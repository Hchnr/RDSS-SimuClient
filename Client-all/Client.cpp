...
int Client::Run()
{
    ...
    while ( true ) {
        int event_num = m_epoll.Wait();
        const struct epoll_event* ready_events = m_epoll.ReadyEvents();
        //handle events
...
void Client::handleEvent(int socket_fd)
{
   ...
   if(socket_fd == 0) {
	   ...
	   int toid, beam;
	   bool isRet, isCnf;
	   char optString[BUFFER_LENGTH];

	   scanf("%s", optString);
	   if(strcmp(optString, "send") == 0) {
	       scanf("%d %d %d %s", &isRet, &isCnf, &toid, buf);
	       printf("[Client recv send from stdin.] %s %d %d %d %s\n", optString, isRet, isCnf, toid, buf);
	       userSend(isRet, isCnf, toid);
	   
	   }
	   else if(strcmp(optString, "move") == 0) {
	       scanf("%d", &beam);
	       printf("[Client recv move from stdin.] %s %d\n", optString, beam);
		   userMove();
	   
	   }
   }
   else if (socket_fd == m_udpsocket.socket_fd()) {
       /* input from UDP socket */
       printf("[Client recv from udp.] \n");
	   int iLen = m_udpsocket.RecvFrom(buf, BUFFER_LENGTH, &from_ip, &from_port);
       if(iLen < 0)
       {
           printf("[client recvfrom(): error] %d when recv.\n", iLen);
           return;
       }
...
int Client::userMoveWait()
{
    ...
    while(1) {
        int iRecv = m_udpsocket.RecvFrom(buf, BUFFER_LENGTH, &srcIP, &srcPort);
        if(iRecv < 0)
        {
...
int Client::userMove()
{
    ...
    *pStamp = (unsigned int) htonl(stamp);
    ...
    int iSend = m_udpsocket.SendTo(buf, packet_len, m_channelIP.data(), m_channelPort);
    if (iSend < 0)
    {
...
int Client::Init()
{
	...
	m_udpsocket.Create();
	if (!m_udpsocket.Bind("162.105.85.118", m_clientPort))
	{
        printf("Client Init new UDP socket error!\n");
        return -1;
	}
    m_udpsocket.SetBlocking(true); //set socket in no block mode
    if (! m_epoll.AddEvent(m_udpsocket.socket_fd(), EPOLLIN)){ //add socket fd to epoll events
        printf("Epoll add udp event failed");
    }
    if (! m_epoll.AddEvent(0, EPOLLIN)){ //add socket fd to epoll events
        printf("Epoll add stdin event failed");
    }
...
int Client::readInitFile()
{
    ...
    m_channelIP = buf;
	m_channelIPint = inet_addr(m_channelIP.data());
